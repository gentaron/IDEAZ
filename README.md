...
21:00 UTC（MYT 5:00）  Cron
  ↓
scripts/sync-published note.com の過去記事の見出しを取ってくる
                       → memory/published.json
  ↓
src/harvest.mjs        鍵の要らない公開の口から候補を集める
                       HN / Reddit / Hugging Face / GitHub / arXiv / 各社のRSS
  ↓
src/published.mjs      既出と当たる候補を、AIに見せる前に機械で落とす
  ↓
...
公開先の過去記事を毎朝取ってきて、**二枚の網**で弾く。

```
note.com/humble_bobcat51
note.com/gensnotes
        ↓ 見出しを全部取ってくる（公開APIは鍵不要。駄目ならRSS）
memory/published.json
        ↓
一枚目（機械）  見出しから固有名詞を抜いて、当たった候補はAIに見せる前に捨てる
二枚目（AI）    残ったものに過去記事の見出しを添えて、言い換えの被りを落とさせる
```

一枚目が効くのは、固有名詞が確実だから。`Qwen3-4B` の記事を書いていれば、
`Qwen3-4B` を含む候補はその時点で消える。AIの気分に左右されない。

ただし `laptop` や `backend` のような、どの記事にも出てくる語で当てると候補が全部消える。
そこで**ありふれ度**を数えて、広く出てくる語は固有名詞とみなさない。
本数が増えるほどこの判定は正確になる（手で除外語を足す必要がない）。

言い換えの被り（同じ話を別の名前で書いたもの）は機械では拾えないので、二枚目に回す。

**同期できていないときは、題材を決めない。** `accounts` が入っているのに `titles` が空なら、
探索は自分で止まる。既出を踏むくらいなら、その日は角度だけの5枠を出す方がいい。

| ファイル | 何が入るか |
| --- | --- |
| `memory/published.json` | `accounts`（手で足す）と、取ってきた見出し。**公開先を増やすならここ** |

```bash
node scripts/sync-published.mjs          # 差分だけ取る
...
| `memory/published.json` | すでに公開した記事の見出しと、その公開先 |
